...
module BootKit
  ...
  class ImportGpg
    ...
    # Initializes the GPG key importer with configuration from bootkit.yml
    # 
    # Sets up 1Password vault, GPG key path, and optional key ID from config
    # Configures logging level if specified in config
    def initialize
      @config = load_config
